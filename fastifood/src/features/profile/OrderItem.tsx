    ...
    return (
        <View style={[styles.container, {borderTopWidth:index === 0 ? 0.7 : 0}]}>
            <View style={styles.flexRowBetween}>
                ...
                <CustomText variant='h7' fontFamily={Fonts.Medium} style={{textTransform:'capitalize'}}>
                    {item.status}
                </CustomText>
            </View>
            ...
            <View style={styles.flexRowBetween}>
                <View style={{width: '50%'}}>
                    {item?.items?.map((i,idx)=>{
                        return (
                          <CustomText variant='h7' numberOfLines={1} key={idx}>{i.count}x {i.item.name}</CustomText>
                        )
                    })}
                </View>
